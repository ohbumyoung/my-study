/*
 SQL
 * D : DATA / L : LANGUAGE
 -DQL (QUERT, 데이터 질의 언어) : 데이터 조회, SELECT (조회)
 -DDL (DEFINITION, 데이터 정의 언어) : 구조(규칙) 관리 , CREATE(생성), ALTER(변경), DROP(삭제)
 -DML (MANIPULATION, 데이터 조작 언어) : 데이터 관리(조작), INSERT(추가), UPDATE(변경), DELETE(삭제)
 -TCL (TRANSACTION CONTROL, 트랜잭션 제어 언어) : COMMIT(적용), ROLLBACK(취소)
 -DCL (CONTRIL, 데이터 제어 언어) : GRANT(권한 부여), REVOKE(권한 회수)
*/
--------------------------------------------------------------------------------
/*
 DML (데이터 조작 언어) : 테이블에 테이터(값)를 추가하거나(INSERT), 변경하거나(UPDATE), 삭제(DELETE) 하기 위해 사용하는 언어
*/
-- INSERT : 테이블에 새로운 행을 추구하는 구문
/*
    [1] INSERT INTO 테이블명 VALUES (값, 값, 값, ...)
    -> 테이블의 모든 컬럼에 대한 값을 직접 제시하여 추가할 때 사용, 컬럼 순서에 맞게 값나열
    (컬럼이랑 동일한 데이터로 제시해야 됨)
    값 부족할때 오류 -> NOT ENOUGH VALUES
...
INSERT INTO EMPLOYEE VALUES (900, '오범영', '980508-1111111', 'dhqjadud@kh.or.kr', NULL, 'D4', 'J4', 4000000, 0.3, NULL, SYSDATE, NULL, 'N');
SELECT *
FROM EMPLOYEE;
...
*/
-- EMP01 테이블 추가 (EMP_ID, EMP_NAME, DEPT_TITLE)
CREATE TABLE EMP01 (
    EMP_ID VARCHAR(3),
...
/*
    [4] INSERT ALL : 두 개 이상의 테이블에 각각 테이터를 추가할 때 사용
                     사용되는 서브쿼리가 동일한 경우 적용하는 방법
    표기법: INSERT ALL
            INTO 테이블명1 VALUES (컬럼명, 컬럼명, ...)
            INTO 테이블명2 VALUES (컬럼명, 컬럼명, ...)
            ( 서브쿼리 )
            => VALUES 다음에 작성하는 컬럼명은 서부쿼리의 결과(RESULT SET)에 표시되는 컬럼명이어야 함
*/
-- EMP_DEPT 테이블 : EMP_ID, EMP_NAME, DEPT_CODE, HIRE_DATE
DROP TABLE EMP_DEPT;
...
-- EMP_MANAGER 테이블 : EMP_ID, EMP_NAME, MAMAGER_ID
...
-- 직원번호가 900번인 직원의 급여를 500만원으로 변경
UPDATE EMP_TABLE 
SET SALARY = 5000000
WHERE EMP_ID = '900';
...
/*
 ...
 WHERE 조건
*/
-- 방명수 직원의 급여와 보너스를 유재식 직원의 급여와 보너스와 동일하게 변경
UPDATE EMP_TABLE 
...
WHERE EMP_NAME = '윤은해';
--지역에서 근무중인 직원들의 보너스를 0.3으로 변경
-- [1]ASIA 지역에서 근무중인 직원 조회
SELECT *
FROM LOCATION
